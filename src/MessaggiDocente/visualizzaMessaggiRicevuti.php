...
try {
    $stmt = $pdo->prepare("CALL VISUALIZZA_MESSAGGI_RICEVUTI_DOCENTE(:email_docente)");
    $stmt->bindParam(':email_docente', $_SESSION['email'], PDO::PARAM_STR);
    $stmt->execute();
    ...
    <?php if (count($messaggi) > 0): ?>
        <?php foreach ($messaggi as $messaggio): ?>
            <div class="messaggio">
                ...
                <p><strong>Da:</strong> <?php echo htmlspecialchars($messaggio['email_studente']); ?></p>
